...
trait Http4sMUnitTestNameCreator {
  ...
  /** Updates the result generated by `nameFor` by using the provided function. */
  def andThen(
      f: (String, Option[Request[IO]], List[String], TestOptions, Http4sMUnitConfig) => String
  ): Http4sMUnitTestNameCreator = (request, followingRequests, testOptions, config) => {
    val previous = nameFor(request, followingRequests, testOptions, config)

    f(previous, request, followingRequests, testOptions, config)
  }
...
object Http4sMUnitTestNameCreator {
  ...
  /** Generates test names like:
    ...
    * }}}
    */
  val default: Http4sMUnitTestNameCreator = (request, followingRequests, testOptions, config) => {
    val names = followingRequests.+:(testOptions.name).filter(_.nonEmpty)

    val joined = names match {
      case Nil                 => ""
      case List(head)          => head
    ...
    request match {
      case Some(request) =>
        val clue = if (joined.isEmpty) "" else s" ($joined)"
        s"${request.method.name} -> ${Uri.decode(request.uri.renderString)}$clue$reps"
      case None if testOptions.name.isEmpty =>
        Assertions.fail("Setting an alias (`.alias(...)`) is mandatory when using a deferred request")
      case None =>
        s"$joined$reps"
    }
  }
